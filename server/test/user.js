...
describe('Create User', function() {
	describe('post users/create', function() {
		it('response should be object and status is 200', function(done) {
			...
			chai.request(app).post('/users/create').send(newUser).end(function(err, res) {
...
describe('Fail Create', function() {
	describe('post users/create', function() {
		it('response should be 400 with validation', function(done) {
			...
			chai.request(app).post('/users/create').send(failInput).end(function(err, res) {
				expect(res).to.be.an('object');
...
describe('Login User', function() {
	describe('post users/login', function() {
		it(`response should be object with status 200`, function(done) {
			...
			chai.request(app).post('/users/login').send(login).end(function(err, res) {
				expect(err).to.be.null;
				expect(res).to.be.an('object');
				expect(res).to.have.status(200);
				done();
...
describe('Empty Input', function() {
	describe('post users/login', function() {
		it(`response should be object with status 400`, function(done) {
			chai.request(app).post('/users/login').end(function(err, res) {
				expect(err).to.be.null;
				expect(res).to.be.an('object');
				expect(res).to.have.status(400);
				done();
...
describe('Validate User Same Email', function() {
	describe('post users/create', function() {
		it('response should be object and status is 200', function(done) {
			let newUser = {
				name: 'Hullaa',
				address: 'Barito 1234567890',
				email: 'admin@mail.com',
				password: 'none12345',
				role: 'customer'
			};
...
describe('Validate Required Input', function() {
	describe('post users/create', function() {
		it('response should be object and status is 200', function(done) {
			let newUser = {};
			chai.request(app).post('/users/create').send(newUser).end(function(err, res) {
